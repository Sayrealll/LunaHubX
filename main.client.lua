...
local KENHUB_KEY_PAGE = "https://jinwoosung555.github.io/Keyy/"
...
local function kenHubKeyGate()
    ...
    if cachedKey and cachedExpiry then
        return true
    end
    ...
    card.Size = UDim2.fromOffset(440, 285)
    ...
    sub.Text = "Get a temporary key from the official key page.\nOne redeemed key stays valid for 24 hours."
...
local function oM(...)
    ...
    do
        ...
        if ok then
            ...
            if y:IsStudio() or not writefile then
                WindUI = require(j:WaitForChild("WindUI"):WaitForChild("Init"))
            else
        ...
        if not sent then
            pcall(function()
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = tostring(data.Title or "Ken Hub"),
    ...
    if WindUI then
        ...
        -- Keep the existing localization/update helpers compatible.
        hk = MainTab
        Ok = ExploitTab
        Yk = AimbotTab
        Tk = ESPTab
        ...
        Fk.secModes = MainTab:Section({
            Title = "Auto Steal Modes"
        })

        Fk.togTween = MainTab:Toggle({
            Title = "Auto Steal (Tween)",
            Desc = "Fly to steal eggs & auto stash into backpack",
        ...
        Fk.togTeleport = MainTab:Toggle({
            Title = "Auto Steal (Teleport)",
            Desc = "Teleport to steal eggs in continuous loop",
            Icon = "solar:magic-stick-3-bold",
            Value = h.autoFarmLoop,
            Callback = function(value)
                if value then
                    T4("WARP")
                elseif Y4 == "WARP" or h.autoFarmLoop then
                    T4("NONE")
                end
            end,
        })
        ...
        MainTab:Button({
            ...
            Desc = "Teleport to steal 1 target egg and return",
            Icon = "solar:magic-stick-3-bold",
            Callback = function()
                task.spawn(function()
                    if Y4 ~= "NONE" then
        ...
        Fk.btnPlaceEgg = MainTab:Button({
            Title = "Place Egg",
            Desc = "Tween home, place all carried eggs & hatch",
            Icon = "solar:box-bold",
            Callback = function()
                task.spawn(function()
                    notify({
                        Title = "Steal An Egg V1",
        ...
        Fk.togAutoPlaceEvery5 = MainTab:Toggle({
            Title = "Auto Place (Every 5)",
            Desc = "Return home every 5 steals, place & wait 5s",
            Icon = "solar:box-minimalistic-bold",
            Value = h.autoPlaceEvery5,
            Callback = function(value)
                h.autoPlaceEvery5 = value

                if not value then
                    h.batchStealCount = 0
                end

                local lang = Gk[Xk] or Gk.EN
                notify({
                    Title = "Auto Place (Every 5)",
                    Content = value
                        and (lang.Notifications.AutoPlaceStarted or "Auto Place (Every 5) enabled")
                        or (lang.Notifications.AutoPlaceStopped or "Auto Place (Every 5) disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togAutoHatch = MainTab:Toggle({
            Title = "Auto Hatch",
            Desc = "Automatically hatch ready eggs continuously",
            ...
            Callback = function(value)
                ...
                notify({
                    Title = "Auto Hatch",
                    Content = value and Gk[Xk].Notifications.HatchStarted
                        or Gk[Xk].Notifications.HatchStopped,
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togAutoReturn = MainTab:Toggle({
            Title = "Auto Return",
            Desc = "Automatically return to safe area after stealing",
            ...
            Callback = function(value)
                ...
                notify({
                    Title = "Auto Return",
                    Content = value and Gk[Xk].Notifications.ReturnStarted
                        or Gk[Xk].Notifications.ReturnStopped,
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togAutoTreadmill = MainTab:Toggle({
            Title = "Auto Treadmill",
            Desc = "Run on base treadmill when no target eggs are spawned",
            Icon = "solar:running-bold",
            Value = h.autoTreadmill,
            Callback = function(value)
                h.autoTreadmill = value
                x()
                n4()

                if not value and (h.onTreadmill or L4()) then
                    M4()
                end
                ...
                notify({
                    Title = "Auto Treadmill",
                    Content = value
                        and (lang.Notifications.AutoTreadmillStarted or "Auto Treadmill enabled (Runs when idle)")
                        or (lang.Notifications.AutoTreadmillStopped or "Auto Treadmill disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togAutoUpgradeTreadmill = MainTab:Toggle({
            Title = "Auto Upgrade Treadmill",
            Desc = "Automatically upgrade base treadmill tier when you have enough cash",
            Icon = "solar:double-alt-arrow-up-bold",
            Value = h.autoUpgradeTreadmill,
            Callback = function(value)
                h.autoUpgradeTreadmill = value
                x()

                local lang = Gk[Xk] or Gk.EN
                notify({
                    Title = (Xk == "TH") and "อัปเกรดลู่วิ่ง" or "Upgrade Treadmill",
                    Content = value
                        and (lang.Notifications.UpgradeTreadmillStarted or "Auto Upgrade Treadmill enabled")
                        or (lang.Notifications.UpgradeTreadmillStopped or "Auto Upgrade Treadmill disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togAutoBuyTrails = MainTab:Toggle({
            Title = "Auto Buy & Equip Trails",
            Desc = "Automatically purchase and equip the best speed trail available",
            Icon = "solar:fire-bold",
            Value = h.autoBuyTrails,
            Callback = function(value)
                h.autoBuyTrails = value
                x()

                local lang = Gk[Xk] or Gk.EN
                notify({
                    Title = (Xk == "TH") and "ซื้อ Trail" or "Buy Trails",
                    Content = value
                        and (lang.Notifications.BuyTrailsStarted or "Auto Buy Trails enabled")
                        or (lang.Notifications.BuyTrailsStopped or "Auto Buy Trails disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })
        ...
        local reverseZoneMap = {}
        for label, name in pairs(zoneMap) do
            reverseZoneMap[name] = label
        end

        local selectedZoneLabels = {}
        for name, selected in pairs(h.selectedZones or {}) do
            if selected and reverseZoneMap[name] then
                table.insert(selectedZoneLabels, reverseZoneMap[name])
            end
        end

        Fk.secEggZones = ExploitTab:Section({
            Title = "Target Zones"
        })

        Fk.dropTargetZones = ExploitTab:Dropdown({
            Title = "Selected Zones",
            Desc = "Choose which zones to farm eggs from",
            Values = zoneLabels,
            Value = selectedZoneLabels,
            Multi = true,
            Callback = function(value)
                local selected = {}

                local function addZone(item)
                    if type(item) == "table" then
                        item = item.Title or item.Name or item[1] or ""
                    end

                    item = tostring(item or "")
                    local zone = zoneMap[item]

                    if not zone then
                        for _, name in ipairs(M) do
                            if string.find(string.lower(item), string.lower(name), 1, true) then
                                zone = name
                                break
                            end
                        end
        ...
        Fk.secEggRarity = ExploitTab:Section({
        ...
        for label, name in pairs(rarityMap) do
            reverseRarityMap[name] = label
        end

        local selectedRarityLabels = {}
        for name, selected in pairs(h.selectedRarities or {}) do
            if selected and reverseRarityMap[name] then
                table.insert(selectedRarityLabels, reverseRarityMap[name])
            end
        end

        Fk.dropTargetRarities = ExploitTab:Dropdown({
            Title = "Selected Rarities",
            Desc = "Choose which rarities to collect",
            Values = rarityLabels,
            Value = selectedRarityLabels,
            Multi = true,
            Callback = function(value)
                local selected = {}

                local function addRarity(item)
                    if type(item) == "table" then
                        item = item.Title or item.Name or item[1] or ""
                    end
        ...
        Fk.togAlwaysSecret = ExploitTab:Toggle({
            Title = "Always Collect Secret+",
            Desc = "Always collect Secret and higher rarity eggs",
            Icon = "solar:fire-bold",
            Value = h.alwaysCollectSecretPlus,
            Callback = function(value)
                h.alwaysCollectSecretPlus = value
                x()
            end,
        })

        AimbotTab:Section({
            Title = "Character & Safety"
        })

        Fk.togGodmode = AimbotTab:Toggle({
            Title = "Godmode",
            Desc = "Invincible against attacks and guards",
        ...
        Fk.btnUnstick = AimbotTab:Button({
            Title = "Get Out Treadmill",
            Desc = "Instantly escape from treadmill or gear",
        ...
        Fk.secFlight = AimbotTab:Section({
            Title = "Flight Settings"
        })

        Fk.sliderSpeed = AimbotTab:Slider({
            Title = "Flight Speed",
            Desc = "Adjust cruise flight speed (studs/second)",
            Step = 25,
            Value = {
                Min = 100,
                Max = 1000,
        ...
        Fk.paraLiveDash = ESPTab:Paragraph({
            Title = "Live Dashboard",
            Desc = string.format(
                "Status: Ready\nFarm Mode: Idle\nCarried Eggs: %d\nFlight Speed: %d Studs/s",
                y4(),
        ...
        Fk.secUI = ESPTab:Section({
            Title = "UI Customization"
        })

        Fk.dropLang = ESPTab:Dropdown({
            Title = "Language",
            Values = {"English", "ไทย"},
            Value = (Xk == "EN" and "English" or "ไทย"),
            Callback = function(value)
                local lang = (value == "ไทย") and "TH" or "EN"
                ...
                -- Keep the tab names from the supplied new UI.
                pcall(function()
                    MainTab:SetTitle("Main")
                    ExploitTab:SetTitle("Exploits")
                    AimbotTab:SetTitle("Aimbot")
                    ESPTab:SetTitle("ESP")
                    InfoTab:SetTitle("Info")
                end)
            end,
        })

        Fk.sliderTransp = ESPTab:Slider({
            Title = "Window Transparency",
            Desc = "Adjust background transparency of the UI window (0% - 90%)",
            Step = 5,
            Value = {
                Min = 0,
                Max = 90,
            ...
            Callback = function(value)
                ...
                pcall(function()
                    ...
                    if Window.AcrylicPaint and Window.AcrylicPaint.Frame then
                        Window.AcrylicPaint.Frame.Visible = (amount == 0)
                    end

                    local background = main:FindFirstChild("Background")
                    if background then
                        if background:IsA("ImageLabel") then
                            background.ImageTransparency = alpha
                        elseif background:IsA("Frame") then
        ...
        Fk.secPerformance = ESPTab:Section({
            Title = "Performance & Graphics"
        })

        Fk.togPerformance = ESPTab:Toggle({
            Title = "Ultra Potato Mode (Maximum FPS Boost)",
            Desc = "Disables shadows, textures, particles, and shaders for maximum FPS smoothness",
            Icon = "solar:bolt-bold",
            Value = h.performanceMode,
            Callback = function(value)
                h.performanceMode = value
                x()
                ...
                notify({
                    ...
                    Content = value
                        and (lang.Notifications.PerformanceStarted or "Performance Mode enabled")
                        or (lang.Notifications.PerformanceStopped or "Performance Mode disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.togDisable3D = ESPTab:Toggle({
            Title = "Disable 3D Rendering (GPU Saver 95%)",
            Desc = "Freezes 3D viewport rendering to drop GPU usage to ~1%. Perfect for overnight farming!",
            ...
            Callback = function(value)
                ...
                pcall(function()
                    y:Set3dRenderingEnabled(not value)
                end)
                ...
                notify({
                    Title = "3D Rendering",
                    Content = value
                        and (lang.Notifications.Disable3DStarted or "3D Rendering disabled (GPU Saver)")
                        or (lang.Notifications.Disable3DStopped or "3D Rendering restored"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.secSystem = ESPTab:Section({
            Title = "System Controls"
        })

        Fk.togAntiAFK = ESPTab:Toggle({
            Title = "Anti-AFK (Double-Esc 10m / Mobile)",
            Desc = "Double-Esc menu pulse every 10m + Mobile touch + PC jitter resets idle timer safely without Idled",
            Icon = "solar:shield-check-bold",
            Value = h.antiAFK,
            Callback = function(value)
                h.antiAFK = value
                ...
                notify({
                    Title = "Anti-AFK",
                    Content = value
                        and (lang.Notifications.AntiAFKStarted or "Safe Anti-AFK enabled")
                        or (lang.Notifications.AntiAFKStopped or "Safe Anti-AFK disabled"),
                    Icon = value and "check-circle" or "x-circle"
                })
            end,
        })

        Fk.btnReset = ESPTab:Button({
            Title = "Reset Character State",
            Desc = "Clear internal states and unlock character movement",
        ...
        Fk.btnRejoin = ESPTab:Button({
            Title = "Rejoin Server",
            Desc = "Reconnect to the same server automatically",
        ...
        Fk.btnUnload = ESPTab:Button({
            Title = "Unload Script",
            Desc = "Completely terminate all loops and close the interface",
        ...
        InfoTab:Button({
            Title = "Reset Character State",
            Desc = "Clear velocity, cancel push & unfreeze",
            Icon = "refresh-ccw",
            Callback = function()
                pcall(D4)
                pcall(u4)
        ...
        InfoTab:Button({
            Title = "Unload Script",
            Desc = "Destroy UI and stop all background loops",
            Icon = "trash-2",
            Callback = function()
                aM()
            end,
        ...
        task.spawn(function()
            while h.alive do
                pcall(function()
                    local carried = y4()
                    local langIsThai = (Xk == "TH")
                    local status = Wk(Xk)

                    if Fk.paraLiveDash and Fk.paraLiveDash.SetDesc then
                        local farmMode = langIsThai and "หยุดพัก" or "Idle"

                        if Y4 == "TWEEN" then
                            farmMode = langIsThai and "ขโมยไข่ (บินเร็ว)" or "Auto Steal (Tween)"
                        elseif Y4 == "WARP" then
                            farmMode = langIsThai and "ขโมยไข่ (วาร์ป)" or "Auto Steal (Teleport)"
                        end

                        local desc = string.format(
                            langIsThai
                                and "สถานะ: %s\nโหมดฟาร์ม: %s\nจำนวนไข่ในตัว: %d ฟอง\nความเร็วบิน: %d Studs/s"
                                or "Status: %s\nFarm Mode: %s\nCarried Eggs: %d\nFlight Speed: %d Studs/s",
                            status,
                            farmMode,
                            carried,
